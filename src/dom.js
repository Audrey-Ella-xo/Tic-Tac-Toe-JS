...
const dom = (function () {
  ...
  // this method renders player score (it needs to be called before the start
  // of the first game and after every finished game)
  ...
  // we will create a custom event that auto-clicks the rest of unclicked fields before we
  // render the new round
  // Disclaimer: by following this approach the console will return errors because we are
  // using once parameter but that won't hinder performance since we are making sure to only
  // dispatsh what it wasn't dispatched by event listener
    ...
    boardField.forEach(field => {
      field.addEventListener('click', (e) => {
        ...
        if (gamePlay.getWinner() !== undefined) {
          ...
          setTimeout(() => { render(3); }, 500);
        }
      }, { once: true });
    });
  };

  // event listening start button of menu view and then render gameplay view
  // and result view when game ends
  const startGame = () => {
  ...
  const deleteRendredScores = () => {
    scoreboardHtml.forEach(playerScore => {
