...
const gamePlay = (function () {
  ...
  const getResult = () => gameBoard.result;
  ...
  const getWinner = () => gameBoard.winner;
  ...
  const checkWinner = (pickedFields, player) => {
    const winConditions = [[1, 2, 3],
      ...
      [3, 5, 7]];
    if (getTurn() < 5) {
      pickedFields; // eslint-disable-line
    } else {
      ...
      // at this case we have two win conditions satisfied at the same turn (753, 159)
  ...
  const play = (domField, players, index) => {
    ...
    domField.background = players[getRole()].getXOField();
    // Stores X/O inside result
    storeXOInsideResult(index);
    // Checking Winner
  ...
  return {
    play, getWinner, resetBoard, getResult,
  };
}());
